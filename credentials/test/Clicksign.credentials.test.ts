...
describe('Clicksign API Credentials', () => {
	describe('properties', () => {
		...
		it('should have correct properties', () => {
			...
			expect(clicksign.documentationUrl).toBe('https://developers.clicksign.com/docs/primeiros-passos');
	...
	describe('authenticate', () => {
		...
		it('should generate a valid query string with access token', async () => {
			...
			expect(authProperty.properties?.qs?.access_token).toBe('={{$credentials.clicksign_access_token}}');
		});
	});
	...
	describe('test', () => {
		...
		it('should have a valid test property', () => {
			...
			expect(clicksignApi.test.request.baseURL).toBe('=https://{{$credentials.clicksign_environment}}.clicksign.com/api/v3/envelopes');
