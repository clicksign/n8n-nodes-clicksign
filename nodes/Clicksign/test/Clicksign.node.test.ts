...
describe('Clicksign Node', () => {
	...
	describe('Node Description', () => {
		...
		it('should have correct request defaults', () => {
			...
			expect(description.requestDefaults).toEqual({
				baseURL: '=https://{{$credentials.clicksign_environment}}.clicksign.com/api/v3',
				url: '',
				headers: {
    ...
    describe('List All Envelopes', () => {
      it('should have listAll operation with correct routing', () => {
        const operationProperty = clicksignOperations.find(op => op.name === 'operation');
        const listAllOption = operationProperty?.options?.find(opt => (opt as any).value === 'listAll') as any;
    ...
    describe('List Documents in an Envelope', () => {
      it('should have envelopeDocuments operation with correct display options', () => {
        const operationProperty = clicksignOperations.find(op => op.name === 'operation');
        const envelopeDocumentsOption = operationProperty?.options?.find(opt => (opt as any).value === 'envelopeDocuments') as any;

        expect(envelopeDocumentsOption).toBeDefined();
        expect(envelopeDocumentsOption?.name).toBe('List Documents in an Envelope');
        ...
        expect(envelopeDocumentsOption?.displayOptions?.show?.resource).toEqual(['envelopes']);
        expect(envelopeDocumentsOption?.displayOptions?.show?.operation).toEqual(['envelopeDocuments']);
      });
    });
    ...
    describe('Create Envelope', () => {
      it('should have createEnvelope operation with correct routing and display options', () => {
        const operationProperty = clicksignOperations.find(op => op.name === 'operation');
        const createEnvelopeOption = operationProperty?.options?.find(opt => (opt as any).value === 'createEnvelope') as any;
        ...
        expect(createEnvelopeOption?.displayOptions?.show?.resource).toEqual(['envelopes']);
        expect(createEnvelopeOption?.displayOptions?.show?.operation).toEqual(['createEnvelope']);
    ...
    describe('List Documents in an Envelope', () => {
      it('should have envelopeId field for envelopeDocuments operation', () => {
        const envelopeIdField = clicksignFields.find(field => field.name === 'envelopeId');
        ...
        expect(envelopeIdField?.displayOptions?.show?.resource).toEqual(['envelopes']);
        expect(envelopeIdField?.displayOptions?.show?.operation).toEqual(['envelopeDocuments']);

        // Check routing
        expect(envelopeIdField?.routing).toBeDefined();
        expect(envelopeIdField?.routing?.request?.url).toBe('=/envelopes/{{$parameter.envelopeId}}/documents');
      });
    });

    describe('Create Envelope', () => {
      it('should have envelopeName field for createEnvelope operation', () => {
        const envelopeNameField = clicksignFields.find(field => field.name === 'envelopeName');
        ...
        expect(envelopeNameField?.displayOptions?.show?.resource).toEqual(['envelopes']);
        expect(envelopeNameField?.displayOptions?.show?.operation).toEqual(['createEnvelope']);
      });

      it('should have locale field with correct options', () => {
        const localeField = clicksignFields.find(field => field.name === 'locale');
      ...
      it('should have autoClose field with correct configuration', () => {
        const autoCloseField = clicksignFields.find(field => field.name === 'autoClose');

        expect(autoCloseField).toBeDefined();
        expect(autoCloseField?.displayName).toBe('Auto Close');
        expect(autoCloseField?.description).toBe('Whether the envelope should be closed automatically when all signers have signed');
      ...
      it('should have remindInterval field with correct configuration', () => {
        const remindIntervalField = clicksignFields.find(field => field.name === 'remindInterval');

        expect(remindIntervalField).toBeDefined();
        expect(remindIntervalField?.displayName).toBe('Remind Interval');
        expect(remindIntervalField?.description).toBe('The interval in days to remind the signer to sign the envelope');
      ...
      it('should have blockAfterRefusal field with correct configuration', () => {
        const blockAfterRefusalField = clicksignFields.find(field => field.name === 'blockAfterRefusal');

        expect(blockAfterRefusalField).toBeDefined();
        expect(blockAfterRefusalField?.displayName).toBe('Block After Refusal');
        expect(blockAfterRefusalField?.description).toBe('Whether the envelope should be blocked after the signer refuses to sign');
      ...
      it('should have deadlineAt field with correct configuration', () => {
        const deadlineAtField = clicksignFields.find(field => field.name === 'deadlineAt');

        expect(deadlineAtField).toBeDefined();
        expect(deadlineAtField?.displayName).toBe('Deadline At');
        expect(deadlineAtField?.description).toBe('The deadline for the envelope');
      ...
      it('should have defaultSubject field with correct configuration', () => {
        const defaultSubjectField = clicksignFields.find(field => field.name === 'defaultSubject');

        expect(defaultSubjectField).toBeDefined();
        expect(defaultSubjectField?.displayName).toBe('Default Subject');
        expect(defaultSubjectField?.description).toBe('The default subject of notification to be sent');
      ...
      it('should have defaultMessage field with correct configuration', () => {
        const defaultMessageField = clicksignFields.find(field => field.name === 'defaultMessage');

        expect(defaultMessageField).toBeDefined();
        expect(defaultMessageField?.displayName).toBe('Default Message');
        expect(defaultMessageField?.description).toBe('The default message of notification to be sent');
	...
	describe('Routing Configuration', () => {
		it('should have correct body structure for createEnvelope operation', () => {
			...
			expect(attributes?.remind_interval).toBe('={{$parameter.remindInterval}}');
			expect(attributes?.block_after_refusal).toBe('={{$parameter.blockAfterRefusal}}');
			expect(attributes?.deadline_at).toBe('={{$parameter.deadlineAt ? DateTime.fromISO($parameter.deadlineAt).setZone($now.zoneName).toISO() : undefined}}');
		});
	});
